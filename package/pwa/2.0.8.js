...
(function () {
  ...
  function process(manifest, urlFactory) {
   ...
   delete last.sizes; // smallest is 'default', no sizes needed
    ...
    function splashFor(_ref, orientation, icon) {
      var width = _ref.availWidth,
          height = _ref.availHeight;
    ...
    function renderBothSplash(applicationIcon) {
      var portrait = splashFor(screen, 'portrait', applicationIcon);
      var landscape = splashFor({
        width: screen.availHeight,
        height: screen.availWidth
      }, 'landscape', applicationIcon);
      // this is particularly egregious setTimeout use,
      // but the .toDataURL() is one of the
  ...
  function shouldUseLightForeground(color) {
    ...
    for (var key in pixelData) {
      var f = pixelData[key] / 255
      pixelData[key] = f < 0.03928 ? f / 12.92 : Math.pow((f + 0.055) / 1.055, 2.4);
    }

    var contrast = Math.abs(1.05 / (getLum(pixelData.r,pixelData.g,pixelData.b) + 0.05));
    return contrast > 3;
  }

  function updateTransparent(image, background, force) {
    if (force === void 0) {
      force = false;
    }
  ...
  function contextForCanvas(_temp) {
